...
function twentyseventeen_child_nav_menu_social_icons( string $item_output, $item, $depth, $args ) {
	...
	if ( 'social' === $args->theme_location ) {
		...
		if ( $social_menu_id === $cur_menu ) {
			$item_output = str_replace( '">', '"><span class="social-links">' , $item_output );
			$item_output = str_replace( '</a>', '</span></a>' , $item_output );
			foreach ( $social_icons as $attr => $value ) {
				if ( false !== strpos( $item_output, $attr ) ) {
					$item_output = str_replace( '<span class="social-links">', '<span class="social-links">' . twentyseventeen_get_svg( array( 'icon' => esc_attr( $value ) ) ), $item_output );
				}
            }
